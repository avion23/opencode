            ...
            const response = yield* http.execute(
              HttpClientRequest.post(route(remoteTarget.url, "/sync/replay"), {
                ...
                body: HttpBody.jsonUnsafe({
                  ...
                  warpID,
                  // The replay body carries the transferring source's current
                  // owner so the destination can authorize a returning warp
                  // (A→B→A) against the owner it recorded during the first leg.
            ...
            const stealResult = yield* Effect.gen(function* () {
              ...
              if (response._tag === "Some") {
                if (response.value._tag === "http-error")
                  return yield* (
                    response.value.status === 409
                      ? new SessionWarpConflictError({
                          message: `Failed to steal session ${input.sessionID} into workspace ${workspaceID}: HTTP 409 ${response.value.body}`,
                          sessionID: input.sessionID,
                        })
                      : new SessionWarpHttpError({
