...
describe("run stream transport", () => {
  ...
  test("propagates an intermediate children lookup failure instead of silently dropping descendant permissions", async () => {
    const src = eventFeed()
    const ui = footer()
    let calls = 0
    const task = createSessionTransport({
      sdk: sdk({
        ...
        children: (async ({ sessionID }) => {
          calls += 1
          if (sessionID === "session-1") return ok([child("child-1")])
          // An SDK error response (not a thrown promise) must still surface;
          // mapping data to [] would silently drop the grandchild's pending
          // permission from the footer.
          return {
            data: undefined,
            error: { name: "NotFoundError", data: { message: "intermediate children lookup failed" } },
            request: new Request("https://opencode.test"),
    ...
    try {
      // The grandchild lookup failure must fail the bootstrap so a pending
      // grandchild permission is not silently dropped from the footer.
      await expect(task).rejects.toMatchObject({
        name: "NotFoundError",
        data: { message: "intermediate children lookup failed" },
      })
    } finally {
      src.close()
    }
