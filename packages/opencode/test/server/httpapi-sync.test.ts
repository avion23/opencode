...
const it = testEffect(Layer.mergeAll(LayerNode.compile(LayerNode.group([Session.node, EventV2.node])), httpApiLayer))
    ...
    () =>
      Effect.gen(function* () {
        ...
        const stale = yield* requestInDirectory(SyncPaths.steal, tmp.directory, {
          method: "POST",
          headers,
          body: JSON.stringify({ sessionID: session.id, seq: 1, warpID }),
        })
        ...
        const committed = yield* requestInDirectory(SyncPaths.steal, tmp.directory, {
          method: "POST",
          headers,
          body: JSON.stringify({ sessionID: session.id, seq: 0, warpID }),
        })
